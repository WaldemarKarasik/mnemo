...
<div id="wrapper" x-data="{show: false, showModal: false, showDelBtn: true, showDelInp:false}" class="h-screen">
    ...
    <h1 class="py-7 text-4xl font-bold text-teal-300 border-b">Admin panel</h1>
    </div>
    ...
    <div class="flex justify-center">
        <div class="pt-16">
    <h1 class="tracking-tight text-3xl leading-tight">Available manipulations with categories:</h1>
    <ul class="py-4 flex space-x-1">
        <li>
            <button
            class="font-semibold cursor-pointer border-2 border-green-500 rounded p-1 transition duration-500 ease-in-out hover:text-green-600 hover:border-green-300 focus:outline-none transform hover:translate-x-2"
        ...
        class="flex w-6/12">
        <li class="
        font-semibold cursor-pointer border-2 border-red-500 rounded p-1 transition duration-500 ease-in-out hover:text-red-600 hover:border-red-300 transform hover:-translate-x-2"
        x-show="showDelBtn" @click="{showDelInp = true, $nextTick(()=>$refs.delete.focus()), showDelBtn=false}">Delete</li>

        <input
        class="py-1 px-3 w-full border-2 focus:outline-none {{($emptyError) ? 'border-red-500' : ''}}" x-show="showDelInp"
        id="deleteInp"  x-ref="delete" wire:model="categoryName" @keydown.enter = "enterDeletePressed()" type="text" placeholder="Category name">
        </div>
        </div>
    </ul>
        </div>
...
<script scoped>
    ...
    window.livewire.on('hideInput',() => {
        document.getElementById('deleteInp').classList.add("hidden")
